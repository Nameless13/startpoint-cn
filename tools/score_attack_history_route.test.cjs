...
const databaseDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "score-history-route-db-"))
...
async function main() {
    ...
    const account = insertAccountSync({
        ...
        idpAlias: "",
        idpCode: "test",
        idpId: `score-history-route-${randomUUID()}`,
        status: "normal",
    })
    ...
    const response = await fastify.inject({
        method: "POST",
        url: "/score_attack_event_battle",
        payload: { viewer_id: viewerId, event_id: 1 },
    })
    assert.equal(response.statusCode, 200, response.body)
    const decoded = unpack(Buffer.from(response.body, "base64"))
    assert.equal(decoded.data.history.length, 1)
    assert.equal(decoded.data.history[0].score, 2_426_000_000)
    assert.equal(Object.keys(decoded.data.history[0]).length, 29)

    const invalidEvent = await fastify.inject({
        method: "POST",
        url: "/score_attack_event_battle",
        payload: { viewer_id: viewerId, event_id: 2 },
    })
    ...
    const invalidViewer = await fastify.inject({
        method: "POST",
        url: "/score_attack_event_battle",
        payload: { viewer_id: 999999999, event_id: 1 },
    })
...
main().then(
    () => console.log("score attack history route tests passed"),
    error => {
        console.error(error)
        process.exitCode = 1
    },
