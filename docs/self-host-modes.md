# 自建服 · 玩「深渊连战 + 15 把深渊武器 + 八位自制/改造角色」完整指南

面向**已经有(或能自己搞到)一套能跑的 WF CN 私服**、想在其上加载两个自制模式、深渊武器和八位自制/改造角色的人。本 mod 只提供**增量**(数据 + 客户端补丁),不含官方基础 CDN。
...
- **本 mod 的增量(①+②两层都在分支里)**:
  ...
  - ② **客户端层**:深渊武器/兑换商店/rush 700099 与角色资源已打包成 `assets/asset-patch/active/` 下的增量链(全部 git 跟踪),服务端 `asset.ts` 会自动 serve;**整链链尾以 `manifest.json` 为准(当前 1.4.312)**。链起点仍是 `cdn_version = 1.4.54`(官方 dump 链尾),`bridge0802` 负责 1.4.54→1.4.90,历史 backfill/rework 负责推进到 1.4.277,最后的 1.4.277→1.4.312 一跳回灌本地已实装终态。**这层缺了 → 打完 15 轮、开兑换商店或加载新角色时会缺主数据/资源**。
  - ⚠️ **前提**:base CDN 至少完整到官方链尾 **1.4.54**；仓库自带桥接边会继续接到 1.4.90 及后续版本。缺少 1.4.54 基线内容时,增量包不能替代约 11GB 的基础 CDN。
  ...
  - ⑥ **本地实装终态层(1.4.277→1.4.312,2026-08-06)**:外科式回灌光龙 151159、暗龙 261089、拉芙 169998、基诺维 169999、700099 连战塔与 15 把深渊武器的本地主数据和资源；另有 1.4.311→1.4.312 兼容入口。每个路径都由冻结合同与发布回执解释为“入包”或“基线已等于终态”,未认领表行保持原基线。
...
运行 `repoint_build.py`。它只改 `DevConfig_gf_android` 的 `IPv4:端口`并用你的 keystore
重签,①③④⑤补丁字节原样保留;构建后还会回读免登录、服务器地址和像素 P-code 标记。
基座里的出厂私有地址不能直接给你的玩家使用。快速工具只支持 IPv4:端口/HTTP;
需要域名、HTTPS或改补丁逻辑时才走下面的高级路线。

> ⑤是通用渲染补丁,会让 `PixelArtCharacterView`、`MemberView`、`CharacterCell`
> 尊重角色资源的 `frame.scale`。它不只服务杰拉德;漏掉时基诺维等新角色也可能出现
> 角色页、队伍格或战斗像素异常放大。旧 APK 和只打①②的 APK 都不算完整客户端。
>
> 签名选择的取舍:**一直沿用同一把 keystore** → 可覆盖安装,本地身份保留;**换签名** →
> 玩家必须卸载重装,本地身份被抹掉、下次登录开新号,存档还在服务端 DB,服主按 `device_id` 重绑。

**高级路线(从官方 APK 手工重做):** 客户端五个补丁缺一不可。用 FFDec 把主 SWF
导出为 AS3 目录(记为 `EXPORT_DIR`),依次应用:
...
# ⑤ 通用逐角色像素占屏(render-scale-v1):让渲染尊重 frame.scale
#    不打的话:杰拉德/基诺维等非官方原始尺寸像素会异常放大
#    站点1=PixelArtCharacterView.as 单行替换(AS3 层,同①-③一起导回);
#    站点2/3=MemberView + CharacterCell P-code;完整流程见 client-patch/render-scale-v1/README.md
```

最后把产出的 SWF 替换进 APK、`zipalign` + `apksigner` 重签名、安装。
`client-patch/abyss-mode-equipment/build_apk.py` 提供了打包+回读验证的一体化脚本(参数见其 README)。

发包前按完整指南核对 `.build-report.json`,再用 `verify_apk_host.py --expect ... --forbid ...`
和 `apksigner verify --verbose --print-certs` 双重回读。最后启动一台客户端,服务端日志实际收到
版本检查/登录请求才算指向成功。

### 5. 发放八位角色(服务端管理后台)

角色数据到位后玩家不会自动拥有,用管理后台邮件发放:打开 `http://<你的服>:8001/admin` → 邮件页 → 附件类型选「角色」,按需分别发送 `129999`(赛瑞斯)、`139999`(史黛拉)、`149999`(杰拉德)、`149998`(风巨蜥)、`151159`(拉夫马诺)、`261089`(阿鲁玛德乌斯)、`169998`(拉芙)、`169999`(基诺维) → 玩家进游戏从邮箱领取。**前提①:发放前先重启服务端**——邮件附件白名单在启动时定格,刚装入的新角色/新装备不重启会被判「不存在于 CDN 数据中」;**前提②:客户端已实际更新到链尾**(以 manifest 为准,当前 1.4.312)——客户端缺资产时领取会崩,且该邮件删不掉、整个邮箱卡死,只能管理后台清空邮箱恢复。

### 6. 进游戏验证

- **模式**:Rush 活动 **700099「深渊连战」** → 打每轮 boss 掉武器/攒代币 → 兑换商店换 15 把深渊武器 → 装上进 700099 / 挑战 2001 / 练习关生效。
- **角色**:邮箱领取 129999/139999/149998/149999/151159/261089/169998/169999 → 角色页确认词条、队长技、立绘和语音可加载 → 分别发动技能确认赛瑞斯双形态、杰拉德强化弹射、光暗龙、拉芙系统入侵与基诺维黑翼/剑型强化弹射资源正常。

---

## 已有部署如何升级(存量服)

已经跑着一个 WF CN 私服(上游 startpoint-cn 或旧版本)想加上两模式、15 把武器与八位自制/改造角色,不用从零来:
...
W=1.4.312      # ← 链尾,以 assets/asset-patch/manifest.json 为准;必须 > V
...
0. **先试现成的「7 表救援包」(免工具,适用于与本链同源的血统)**:
   ...
   **1.4.125 窗口的状态**,已含全部模式行、该窗口的官方键(如 item 10000140)和真机验证的
   ...
   (`assets/rush_event_quest*.json`)是 **1.4.312 配套态**。直接混用 = server/client 两半不同步
...
3. 自制角色不要照抄 charpkg 整表,用角色包工作流(`wf_release` / `wf_character_flow`,
   见工具仓 docs/角色包工作流.md)把所需角色装进你的表;当前发布角色 ID 为
   129999/139999/149998/149999/151159/261089/169998/169999;
...
- **拉夫马诺(151159,光,★5)**:延续爱之龙,以古龙威光降低敌方光属性抗性并消除强化效果。
- **阿鲁玛德乌斯(261089,暗,★5)**:传承心愿的冥龙,三段暗属性技能兼具消除强化、麻痹、降暗抗和队伍辅助。
- **拉芙(169998,暗,★5)**:新春夜宴形态,全屏机甲攻击并赋予敌方「系统入侵」和队伍攻击提升。
- **基诺维(169999,暗,★5)**:破契的黑鸦,以血契、护盾、黑羽光环和剑型强化弹射支援队伍。

## 常见坑

- 客户端**只打了免登录+重定向、漏了深渊门控** → 能进服、能拿到武器,但装上在战斗里不生效。
- 客户端**漏了④双形态补丁**但服务端有角色数据 → 领赛瑞斯没事,但他的特殊演出播放即崩;湿润雷伤加成也不生效。五个补丁都要打(⑤例外:漏了只是杰拉德像素偏大,不崩)。
- **客户端没更到链尾就邮件发自制角色** → 领取后下载/进战崩溃(客户端缺角色资产),邮箱可能整体卡死。先确认玩家客户端已拉到 1.4.312 再发角色。
